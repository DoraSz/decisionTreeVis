...
// given 2 datasets where one of them does not have a convex hull, compute if they intersect/overlap
function intersecting(below, above) {
	// 2 points always separable
	if (below.length == 1 && above.length == 1) {return false;}



	if (below.length == 1 && above.length == 2) { // below is between the 2 points on the line
		if (Math.abs(euclediandistance(above[0], below[0]) + (euclediandistance(below[0], above[1]) - euclediandistance(above[0], above[1]))) < epsilon) {
			return true;
		} else {
			return false;
		}
	}
	if (below.length == 2 && above.length == 1) { // above is between the 2 points on the line
		if (Math.abs(euclediandistance(below[0], above[0]) + (euclediandistance(above[0], below[1]) - euclediandistance(below[0], below[1]))) < epsilon) {
			return true;
		} else {
			return false;
		}
	...
	if (below.length == 2 && above.length == 2) {
		var line1 = pointstoline(below[0], below[1]);
		var line2 = pointstoline(above[0], above[1]);
		var cutpoint = lineintersection(line1, line2);
		if (cutpoint == null) { // parallel lines
			return false;
		} else { 
			var joinedhull = d3.geom.hull(below.concat(above)); // the lines intersect
			if (findPointInsidePolygon(ob(cutpoint), obj(joinedhull))) {
				return true;
			} else {
				return false;
			} 
	...
	if (below.length > 2 && above.length == 1) {
		if (findPointInsidePolygon(ob(above[0]), obj(below))) {
			return true;
		} else {
			return false;
		}
	...
	if (below.length == 1 && above.length > 2) {
		if (findPointInsidePolygon(ob(below[0]), obj(above))) {
			return true;
		} else {
			return false;
		}
	...
	if (below.length > 2 && above.length == 2) {
		// at least one point in the polygon
		if (findPointInsidePolygon(ob(above[0]), obj(below)) || findPointInsidePolygon(ob(above[1]), obj(below))) {
			return true; 
		} else {
			// points cut the polygon
			var cuts = findcuts(above, below).cuts;
			if (cuts.length > 0) {
				// calculate distAB - dist AC - dist BC
				var epsilon = 5.96e-08;
				var distances = euclediandistance(above[0],above[1]) - euclediandistance(above[0],cuts[0]) - euclediandistance(above[1],cuts[0]);
				// check if C is between AB by measuring the distances
				if (distances > 0 && Math.abs(distances) < epsilon) {
					return true;
				}
			} else {
				return false;
	...
	if (below.length == 2 && above.length > 2) {
		if (findPointInsidePolygon(ob(below[0]), obj(above)) || findPointInsidePolygon(ob(below[1]), obj(above))) {
			return true;
		} else {
			// points cut the polygon
			var cuts = findcuts(below, above).cuts;
			if (cuts.length > 0) {
				// calculate distAB - dist AC - dist BC
				var epsilon = 5.96e-08;
				var distances = euclediandistance(below[0],below[1]) - euclediandistance(below[0],cuts[0]) - euclediandistance(below[1],cuts[0]);
				// check if C is between AB by measuring the distances
				if (distances > 0 && Math.abs(distances) < epsilon) {
					return true;
				}
...
// create an object representation for point arrays for using intersection.js
function obj(points) {
  var object = [];
  for (p in points) {
    object.push({x:points[p][0], y:points[p][1]});
  }
  return object; 
}
